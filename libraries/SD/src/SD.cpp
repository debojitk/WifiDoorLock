...
SDFileWrapper SDClass::open(const char *filepath, uint8_t mode) {
  /*
  ...
  if (! filepath[0]) {
    ...
    return SDFileWrapper(parentdir, "/");
  }
  ...
  return SDFileWrapper(file, filepath);
}


/*
SDFileWrapper SDClass::open(char *filepath, uint8_t mode) {
  //
...
SDFileWrapper SDFileWrapper::openNextFile(uint8_t mode) {
  dir_t p;
...
void SDFileWrapper::rewindDirectory(void) {  
  if (isDirectory())
    _file->rewind();
}
